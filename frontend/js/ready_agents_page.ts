/*
Файл с логикой для страницы с готовыми нейросотрудниками
*/

import { getReadyEmployees } from './api_client.js';
import MenuPage from './menu_page.js';
import ChatPage from './chat_page.js';

export default class ReadyAgentsPage {
    private employees: any[] = [];

    async render(): Promise<void> {
        const template = document.getElementById('ready-agents') as HTMLTemplateElement;  
    ...
    private async loadEmployees(): Promise<void> {
        try {
            const data = await getReadyEmployees(); 
            ...
            if (container) {
                container.innerHTML = '<div class="error-message">Не удалось загрузить сотрудников.</div>';
            }
        }
    }
    ...
    private renderEmployees(): void {
        ...
        this.employees.forEach(emp => {
            ...
            console.log('avatarEl found:', avatarEl);
            if (avatarEl) {
