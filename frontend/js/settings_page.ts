...
export default class SettingsPage {
    render(): void {
        const template = document.getElementById('settings-template') as HTMLTemplateElement;
    ...
    private fillUserInfo(): void {
        ...
        if (user) {
            ...
            if (nameSpan) nameSpan.textContent = userData.name || 'Не указано';
            if (emailSpan) emailSpan.textContent = userData.email || 'Не указано';
        } else {
            if (nameSpan) nameSpan.textContent = 'Гость';
            if (emailSpan) emailSpan.textContent = 'Не авторизован';
        }
    }
    ...
    private initThemeSwitcher(): void {
        const themeBtn = document.querySelector('.theme-swicher') as HTMLButtonElement;
        if (!themeBtn) return;
