...
export default class MenuPage {
    ...
    attachEvents() {
        ...
        containerMenu.addEventListener('click', (event) => {
            const buttonMenu = event.target.closest('.menu-button, .menu-button-settings');
            if (!buttonMenu)
                return;
    ...
    async updateBadges() {
        try {
            ...
            const myUnread = (myData.employees || []).reduce((sum, emp) => sum + (emp.count_message || 0), 0);
            const readyUnread = (readyData.employees || []).reduce((sum, emp) => sum + (emp.count_message || 0), 0);
    ...
    updateBadge(action, count) {
        ...
        const dot = button.querySelector('.indicator-dot');
        if (badge) {
            if (count > 0) {
